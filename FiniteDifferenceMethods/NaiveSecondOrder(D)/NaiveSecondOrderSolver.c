...
void evolveh(double *h, double *u, double *ph, double g, double dx, double dt, int nBC, int n,double *nh)
{
    ...
    for (i = nBC ; i < n +nBC;i++)
    {
        chx = 0.5*idx*(h[i+1] - h[i-1]);
        cux = 0.5*idx*(u[i+1] - u[i-1]);
        nh[i -nBC] = ph[i] - 2*dt*(h[i]*cux + u[i]*chx); 
...
void evolvewrap(double *u, double *h, double *pubc, double *phbc, double *h0, double *h1, double *u0, double *u1, double g, double dx, double dt, int nBC, int n, int nBCs)
{
	...
	// phbc is the vector of h at the previous time step with ghost cells over boundaries
    ...
    evolveh(hbc, ubc,phbc,g,dx, dt,nBC, n,h); 
